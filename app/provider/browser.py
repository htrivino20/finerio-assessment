...
class BrowserProvider:
    def __init__(
        self, url: str, playwright: PlaywrightModule = None, openai: OpenAIModule = None
    ):
        self._url = url
    ...
    async def init_bypass_strategy(self):
        ...
        if "demo" in self._url:
            self._captcha_strategy = AudioCaptchaStrategy(
                self._playwright, self._openai, self._url
            )
